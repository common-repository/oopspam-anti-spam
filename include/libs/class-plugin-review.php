 ...
 */
if ( ! class_exists( 'OOPSpam_Plugin_Review' ) ) :
class OOPSpam_Plugin_Review {
	...
	public function display_admin_notice() {
		...
		if ( isset( $screen->base ) && 'plugins' == $screen->base ) {
			...
			<div class="updated notice notice-success">
				<p>' . sprintf( __( 'You have been using the %s plugin for %s now. Please write a review for our plugin. Your review helps us (and motivates us) to push forward with new developments, updates, features and improvements. ♥️', 'oopspam' ), $this->name, $time ) . '
					<br /><br />
					<a onclick="location.href=\'' . esc_url( $no_bug_url ) . '\';" class="button button-primary" href="' . esc_url( 'https://wordpress.org/support/view/plugin-reviews/oopspam-anti-spam/#postform' ) . '" target="_blank">' . __( 'Write a review', 'oopspam' ) . '</a>
					   
					<a href="' . esc_url( $no_bug_url ) . '">' . __( 'No thanks.', 'oopspam' ) . '</a>
				</p>
			</div>';
